...
const getQuiz = async (req, res) => {
    ...
    if(docId === undefined || docId === null) {
        return res.status(400).json("Could not get quiz data due to missing quiz ID in request.")
    }
    ...
    try {
        ...
        if(snapshot.exists) {
            ...
            return res.status(404).json(`Quiz with id ${docId} does not exist.`)
        }
...
const updateQuiz = async (req, res) => {
    ...
    if(docId === undefined || docId === null || quiz === undefined || Object.keys(quiz).length === 0) {
        return res.status(400).json("Could not update quiz data due to missing quiz ID or quiz data in request.")
    }

    try {
        await db.collection('quizzes').doc(docId).update(quiz);
        res.status(200).json(`Successfully updated quiz with id ${docId}`)
    }catch(error) {
        console.error(error)
        if(error.code === 5) {
            console.log(`Document with id ${docId} not found.`)
            return res.status(400).json("Could not update quiz data due to invalid id.")
        }
        return res.status(500).json("ERROR: Database query failed.")
    }
}
